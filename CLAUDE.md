...
- **2026-07-27 (f)** — **Perf runs are archived per run, and the history is committed.** `run.sh` writes into `out/<YYYYMMDD-HHMM>/` and **names the folder itself**, because the first hand-named one was already a day out of step with the `fetchTime` inside it. `emit.mjs` takes the newest folder by default or a named one, so an old batch can be re-emitted instead of being lost, and appends a distilled snapshot to **`scripts/perf/history.jsonl`** — committed, a few KB a line, keyed on run id so a re-emit corrects its line rather than duplicating it. The raw runs stay gitignored at ~30MB a batch. `trend.mjs` reads it. Backfilled with three batches recovered from git, each carrying `recoveredFrom`; a fourth was deliberately left out, being the mixed set that the 07-25 "one batch" guard was written to reject.
  - **What the history immediately settled: there is no page-specific mobile problem.** Across four batches the mobile average sat at 98.3 / 98.8 / 98.5 / 98.4 — half a point of range across three days and several shipped changes — while individual pages moved 4 to 5 score points. **Pages that always score 100 have an LCP spread of 5-11ms; every page that wobbles has a spread of ~600ms.** They are not slower pages, they are pages whose LCP sits near a scoring threshold inside a 600ms noise envelope, so which side they land on is chance. Home has scored 95, 99, 98, 94 on identical-or-better code. **Compare averages, never page scores**, and prefer a CPU metric like TBT, which is what caught the hero decode cost while every LCP number was unreadable.
