...
- **2026-07-27 (b)** — **Home hero re-cut, and the home hero formally split from the per-slug system.** `docs/hero-pipeline.md` claimed home "uses the same shape"; running its recipe against the new master would have scaled a square source to 16:9 and muxed an audio track into a silent clip. Home is square, silent, autoplaying, and hand-wired in `src/pages/index.astro`. The doc now says so and carries its own recipe. It also had a real bug in the shared recipe: **pass 1 ran without the `-vf` scale**, so two-pass stats were gathered at the master's resolution and handed to a pass 2 encoding at another. Both passes take identical filters.
  - **The served filename is versioned now (`hero_900-v2.webm`), because `/hero/*` is `immutable` for a year.** Home is the one hero whose bytes actually get replaced, so a re-cut under the old name reaches new visitors only. Renaming is the remedy `public/_headers` already prescribes; bump to `-v3` next time and delete the superseded file in the same change. None of this touches `src/lib/media.ts` or the eight case studies, because home hardcodes its own paths rather than deriving them from a slug.
  - **The poster moved to `src/assets/hero/home/poster.webp`,** so it is hashed and responsive like the eight case-study posters instead of shipping verbatim from `public/`. It is both the LCP element and the entire hero under reduced motion or save-data, so it renders as a real `<img>`, never the video's `poster` attribute, and it sits *over* the video and is dropped on the first `playing` event: if the script never runs the video never plays either, so the still staying put is the correct failure. **`Base.astro` gained `preloadImageSrcset` + `preloadImageSizes`, and a responsive preload needs both** — a bare `href` against a `srcset` the browser resolves differently downloads the LCP element twice. One `getImage()` call feeds the `<img>` and the preload so the two cannot diverge.
  - **Resolution is the lever on grain, not CRF.** This animation carries newspaper texture, a globe, and film grain, so CRF 40 still landed at 1.39 MiB, while 900x900 at CRF 36 landed at 1.13 MiB and was indistinguishable from the 1080 encode at the size the figure actually renders. Against the old 1.11 MiB that is weight-neutral on the site's heaviest page. Encode a spread and compare at display size; a CRF default is not an answer here.
  - **`sizes` is measured, not guessed.** Nothing in the hero's ancestor chain sets a `max-width`, so the figure is `0.45 * (100vw - 456px)` and keeps growing: 652px at a 1920 viewport, ~950px at 2560. The ladder carries an 1100 step on purpose, because Lighthouse's mobile emulation (412px at DPR 2.625) asks for 1081, and a 1080 to 1440 jump there would hand the LCP element ~16KB instead of ~10KB.
  - The lockup renders "2026", so this one needs a re-cut each January. Perf is not re-measured yet: the poster got smaller at every width and the video slightly larger, so check CLS alongside LCP before restating any published figure.
