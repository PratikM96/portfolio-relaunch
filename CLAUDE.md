...
- **Aug 2026 to present** = "International Student Exchange", role "Director of Marketing", and **Director of Marketing is the single public identity**. No ISE figure is publishable until it is measured; the role's targets are not results.
...
- **2026-08-03** — The independent practice is **closed at Aug 2026**, so `Present` on that entry is wrong wherever it survives, and Director of Marketing at International Student Exchange is the current role and the single public identity. **`src/pages/journal/[slug].astro` sat outside `claims.mjs` TARGETS**, so the author bio it renders on every post was unguarded copy; it is in TARGETS now, and the retired availability phrase fails the build.
