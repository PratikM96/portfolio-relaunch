...
- **The site does not name the current employer outside `/resume`.** Public copy says "a nonprofit" in prose, "Nonprofit" in a label row, the title already carrying "International". `/resume` and the JSON-LD `worksFor` are the two deliberate exceptions, both allowlisted in `scripts/check/claims.mjs`. Same shape as The Ninth's rule above.
...
- **2026-08-09** — The title is **International Marketing Director**, and the employer euphemism drops the word the title now carries: "Nonprofit" in a label row, "a nonprofit" in prose. `claims.mjs` guards the string but **structurally cannot see two surfaces**: the rail splits it across a `<br />`, and `scripts/og/render-cards.mjs` sits outside `TARGETS` while baking it into `public/og/default.png`, which no check and no build step regenerates. The ISE field team size is not in the Claim Registry, so no figure ships until it is.
- **2026-08-03** — The independent practice is **closed at Aug 2026**, with the employer named on `/resume` and in the JSON-LD only; `src/pages/journal/[slug].astro` was outside `claims.mjs` TARGETS, leaving the author bio it renders on every post unguarded.
