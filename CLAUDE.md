...
- **2026-07-28** — Consent is **geo-split**, the opt-in gate inside the EEA, UK and Switzerland and a notice with one-click opt out everywhere else, keyed on a same-origin `/cdn-cgi/trace` fetch where **anything short of a confirmed two-letter non-European code fails closed to the gate**. **Loading on arrival is not loading immediately**: gtag.js is 168KB of third-party main-thread script, and injecting it inline cost /brand ~300ms of TBT and pushed its mobile LCP past 4s, so the queue is primed on arrival and the library fetched at idle behind a `setTimeout` floor — priming first is what keeps `generate_lead` and `demo_interaction` alive, because `track()` checks gtag at click time and an unprimed delay drops the clicks inside it. Since **the consent script ships in every page's `Base.astro` bundle**, that one change moved every page's perf and regressed /brand without /brand being touched; the published JS figure had meanwhile sat at 3.2KB against a 3.8KB build, because **nothing fails the build when a perf claim drifts** and only `npm run perf:js` catches it. The install is now one file, `src/scripts/ga-core.js`, with a generated copy at `public/concepts/ga-core.js` for the microsites, which cannot import from `src/`: **the generator is deliberately not a `prebuild` hook**, since regenerating automatically would silently overwrite a hand edit to the copy, which is the exact failure the arrangement exists to make loud. Also today: `img-src` deliberately does not allow `googletagmanager.com`, so gtag's occasional `/td` image beacon is blocked and **an intermittent best-practices 96 is the CSP working**, not a regression to chase; and **a tsconfig `exclude` REPLACES the inherited array rather than merging**, which had been feeding `dist`'s minified bundles to `astro check`.
