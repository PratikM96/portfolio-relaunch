...
- **2026-08-03** — The independent practice is **closed at Aug 2026**, so `Present` on that entry is wrong wherever it survives, and Director of Marketing is the current role and the single public identity, with the employer named on `/resume` and in the JSON-LD only. **`src/pages/journal/[slug].astro` sat outside `claims.mjs` TARGETS**, so the author bio it renders on every post was unguarded copy; it is in TARGETS now, and the retired availability phrase fails the build.
