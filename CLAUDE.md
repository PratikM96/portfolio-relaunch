...
- **2026-07-28 (g)** — Build targets are pinned to Safari 16.2 / Chrome 111 / Firefox 113, the floor `color-mix()` already sets and the token system depends on. Unpinned, the minifier rewrote `@media` into range syntax needing Safari 16.4, and **an unsupported media condition drops the whole block**, so 16.0 to 16.3 would have been served the desktop layout on a phone. It cost 36 bytes raw, saved 7 gzipped, and left the JS byte-identical.
