...
- **2026-07-27 (e)** — **Mobile's bottleneck was the fonts, so JetBrains Mono is subset now.** With TBT and CLS already at 0, 55% of the mobile score was maxed and every remaining point sat in FCP and LCP — and both were gated by **165KB of `High`-priority woff2 on the critical path of every page**, which at Lighthouse's throttled rate is most of FCP. `LCP ≈ 2 x FCP` sitewide because the LCP element is text that repaints when its face swaps in. The largest face carried the least text: JetBrains Mono is 89KB and renders only the three mono tiers, while Clash carries everything else. Subset to the characters the site can use, **89.3KB to 35.4KB**, so the three faces went 163KB to 109KB. `scripts/fonts/subset-mono.mjs` is the record and §6 carries the rule.
  - **Only this face may ever be subset** — Fontshare grants Clash embedding but not modification, JetBrains Mono is OFL-1.1 with no Reserved Font Name and grants both. §9 (2026-07-15) named this as the first lever to pull; it is now pulled, and there is no second one.
  - **The input is the vendor original in `_reference/`, never the shipped file.** Re-running against an already-subset face works once and then silently narrows the set every time after. The script refuses if the original is missing rather than guessing.
  - **A character set scanned from the built HTML would have been wrong.** `▶` exists only inside a JS string that swaps the hero's play/pause chip, so it appears in no markup and no content file. The set is a fixed floor (all printable ASCII, Latin-1 letters, and a symbol list) rather than a scan, because a missing glyph does not error: it renders in the metric-matched fallback and looks subtly off.
  - **Verified by advance width, which is the test a mono face makes available:** every glyph must share one advance, so a canvas measurement across all 177 requested characters finds any that fell back. 174 present. The three that are not — `❚` `★` `▢` — **are absent from JetBrains Mono itself and always have been**, proven by subsetting the original to just those three and getting 984 bytes, less than the empty shell around a single real glyph. Pre-existing, not a regression, and documented in the script so it is not mistaken for one.
  - **Home stopped fetching four below-the-fold card posters (34KB).** A `poster` attribute fetches the instant it exists, so `card-video.ts` now applies posters ~300px before the card scrolls in, the same margin OutputGrid already used for its video posters. `/work`'s first featured card keeps `poster` in its markup on purpose: it is that page's preloaded LCP element, and deferring it to script would delay the paint the preload exists to hurry.
  - **Tried and reverted: splitting PerfTable and EmbeddedDemo CSS out of the shared case-study bundle.** Astro drops an unrendered component's script but keeps its CSS, so both ship to all 12 case studies while rendering on 1 and 3. A dynamic import is the fix and `astro check` rejects it as a possibly-null component. The whole prize is ~2.4KB gzipped of 8.2KB, about 10ms throttled, so it was not worth a type cast. Measured and left alone; the reasoning sits above the imports in `[slug].astro`.
  - Not done, and deliberately: **home still ships 1.15MB of hero video to mobile.** It costs no score (it starts after load and TBT is 0) but it is the largest real-user cost on the site, and the `frugal` guard only catches save-data and 2g/3g, so a 4G phone downloads all of it. Whether the loop should autoplay at phone widths at all is a design call, not a perf one.
