...
- **Aug 2026 to present**, role "International Marketing Director", and it is **the single public identity**. No figure from it is publishable until it is measured; the role's targets are not results, and the field team size is not in the Claim Registry.
- **The current employer is named.** Prose says "International Student Exchange"; the home and closing-CTA label rows say "ISE", where the line is tightest. `scripts/check/claims.mjs` fails the build on the euphemism, with a pattern narrow enough that JMTP's and SR Love and Care's own nonprofit copy is unaffected.
...
- **2026-08-17** — The employer is named: "International Student Exchange" in prose, "ISE" in the two label rows, with `claims.mjs` failing the build on the euphemism through a pattern narrow enough to leave JMTP's and SR Love and Care's own nonprofit copy alone. `/resume`'s ISE bullets and summary now track the Resume Master verbatim, which is what carries the registry's "11.7M cross-platform" wording onto the page. **The rail and mobile drawer carry the title only**, their `<br />` splitting "International / Marketing Director" rather than the employer, so identity rows in `Base.astro` are not an employer surface.
- **2026-08-09** — The title is **International Marketing Director**, and the ISE field team size is not in the Claim Registry, so no figure ships until it is. `scripts/og/render-cards.mjs` bakes the title into `public/og/default.png` from outside `TARGETS`, and no check or build step regenerates it.
- **2026-08-07** — Ported to macOS, and the audit of everything marked "contract" mattered more than the port. **`public/fonts/OFL.txt` was not the OFL**: a scraped page saved as text, every soft wrap deleted without a space (`must not bedistributed`), one `&` left as `&amp;`, and a wrong copyright line, with `_reference/`'s "vendor original" byte-identical to it, so the usual restore-from-vendor rule could not fix it. **Poster stills live in `src/assets/`, never beside their clips**, and two docs still described the old `public/` layout, so following either recipe put the file where nothing reads it and failed the build; the concept microsites were outside `claims.mjs` TARGETS the same way, leaving "live AI moment-clipper" shipped against a rule that already banned it. Homebrew's slim `ffmpeg` has **no libwebp** while `ffmpeg-full` does, and a `.gitignore` entry with a **trailing slash matches directories only**, so `_reference/` silently stopped ignoring a symlink that a Windows junction had satisfied.
- **2026-08-03** — The independent practice is **closed at Aug 2026**, and `src/pages/journal/[slug].astro` sits in `claims.mjs` TARGETS because the author bio it renders on every post is copy like any page's.
