...
**Licensing is a build constraint, not a footnote.** The repo is public and the fonts ship in it, so every face must permit both web embedding and subsetting. JetBrains Mono is OFL-1.1 with no Reserved Font Name (modification allowed, name may be kept); its licence ships at `public/fonts/OFL.txt` because OFL §2 requires the licence to accompany the font. Before adding any face, read its licence and confirm both rights. Berkeley Mono (TX-02) was removed on 2026-07-15 for exactly this reason — no web licence held.
