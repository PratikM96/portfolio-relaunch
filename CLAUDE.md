...
- **2026-07-28 (f)** — The mobile drawer sizes to `100dvh`, because a fixed `inset: 0` resolves against the LARGE viewport and put the bottom-pinned readout under a dynamic toolbar, so the drawer scrolled for a reason no visitor could see. A `max-height` query compresses the nav rhythm on short viewports, which is a height axis and not a fifth breakpoint. **The CSS minifier rewrites `max-width` into range syntax (`width<=900px`)**, so grepping the build for the authored form finds nothing and proves nothing.
