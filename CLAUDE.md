...
| Metric wording, retired claims, style defaults | `scripts/check/claims.mjs` — the schema checks shape, this checks values, and `npm run build` runs it first |
...
- **2026-07-28 (e)** — /work's first featured card sets its poster from an `is:inline` script instead of a static attribute, which was stuck on the dark still in light theme on every fresh load. **A `<video poster>` cannot be theme-aware and `card-video.ts` only ever *adds* a poster**, so a hardcoded one is permanent until a live theme flip; `rel=preload` cannot be theme-conditional either, so /work no longer preloads it and its LCP needs re-measuring.
- **2026-07-28 (d)** — The Claim Registry said `claims.mjs` enforced the retired "zero layout shift" and the brand-systems count; neither rule existed, so both are now real. **A master asserting enforcement is not enforcement**: check the file before believing the sentence.
- **2026-07-28 (c)** — The /work brands leader row puts names on their own full-width line, after the shared-row version wrapped even at 1440. **A preview harness without the shell inset overstates the content column by the rail plus its padding**, so a layout validated in one can still be wrong on the page.
...
- **2026-07-27 (e)** — JetBrains Mono subset (89KB → 35KB) after 165KB of High-priority woff2 gated mobile FCP/LCP; the rules keeping the subset honest are §6. Two things measured and deliberately **not** done, so don't re-propose them: splitting `PerfTable`/`EmbeddedDemo` CSS out of the case-study bundle (~2.4KB gzipped, reasoning sits above the imports in `[slug].astro`), and dropping home's 1.15MB hero video on mobile, which costs no score but is the largest real-user cost on the site.
- **2026-07-27 (d)** — The "zero layout shift" claim retired: a perfect Lighthouse score already entails a good CLS, so it was a second number saying the same thing and the only one that could rot. `PerfTable` still renders per-page CLS as measured detail; the Resume Master carries a dated note not to reinstate it.
- **2026-07-27 (c)** — Layout became a scale (`src/lib/layout.ts`), after five hand-maintained `sizes` strings drifted and the two written by hand were the two that were wrong. **`frc` is a 720p source, not a bad export** — leave its hero alone.
- **2026-07-27 (b)** — Home's hero re-cut and split from the per-slug system: square, silent, autoplaying, hand-wired, and **versioned** (`hero_900-v2.webm`) because it is the one hero whose bytes get replaced under a year of `immutable`. Recipe and versioning rule in `docs/hero-pipeline.md`.
- **2026-07-27** — American spelling adopted and enforced (§4), after `/brand` shipped "Color" in one heading and "Colour" in another.
- **2026-07-26 (d)** — Motion rebuilt as a semantic system: six durations renamed for intent, values unchanged, plus `--travel-*` / `--scale-*` / `--stagger-step` because transform magnitudes were literals that had drifted. Page transitions are the native CSS `@view-transition`, **never Astro's `<ClientRouter />`**, which would spend the JS budget the site publishes a number for.
- **2026-07-26 (c)** — The reveal observer's `threshold` is 0 (§6). Verifying motion in a browser: **an unfocused or occluded Chrome tab throttles rendering and IntersectionObserver stops firing**, so screenshot first, then read the DOM. Extension-injected JS runs in an isolated world where IO never fires at all.
...
- **2026-07-25 (c)** — Small-screen pass below 1100. Traps it exposed, all now in §6: a token typo fails silently inside a shorthand, `.rev` can't reveal a tall above-the-fold block, and a bare `1fr` track floors at min-content. Two that live only here: **`cqi` with no `container-type` ancestor resolves against the viewport**, so §6's container-query exemption isn't actually wired up anywhere and isn't precedent; and **`astro preview` outlives `npm run preview`**, leaving a `workerd.exe` child holding `dist/client`, so kill the PID and verify `dist` deleted before believing a rebuild.
- **2026-07-25 (b)** — Audit sweep after 13 metric figures were found inflated against the Resume Master, every one upward; values moved under `scripts/check/claims.mjs`. **Machine-readable surfaces need the same copy sweep as pages** — `llms.txt` was asserting a rule retired the day before.
- **2026-07-25** — The remaining axes got scales (`--fs-*`, `--fs-fluid-*`, duration / radius / scrim / shadow). Deliberately **not** done: a `.meta-label` primitive for the 38 rules reading `--fs-2xs / uppercase / --text-muted`, since they already resolve to the same tokens.
- **2026-07-24** — The concept "scope, never results" carve-out retired: a shipped concept follows the same case-study rules, accent included, still gated by the honesty rule. Disclosure stays required.
- **2026-07-24 (c)** — Output width ladders are device pixels and Astro never upscales (§8).
- **2026-07-24 (b)** — The site ships as its own case study (`portfolio-system`). Two one-offs bespoke to that entry, so don't generalize them: `perfTable: true`, and an Output section that is a device x theme matrix rather than a `longpage`.
- **2026-07-23** — Mobile weight pass + metric-matched font fallbacks. **CLS was 0 only because the page was slow** — cutting ~100ms off FCP exposed a 0.185 font-swap shift, so check CLS on every perf change, not just the metric being optimized.
- **2026-07-22** — System audit sweep: removed `cover`, the placeholder subsystem, the mp4 fallback and the `cardVideo` flags; adopted the type and spacing scales, which had near-zero uses.
- **2026-07-15** — Type scale adopted (§6); before it, h1/h2/h3 disagreed on family and mono labels carried seven trackings. **Wordmark = `h1` tier**, sentence case, and `h1`'s only consumer: **display weight and tracking are optical compensations for large type, never for UI-scale text.** Fonts became variable, one file per family. JetBrains Mono replaced Berkeley Mono (no web license held, repo is public); metrics matched, so the swap was layout-neutral. Work facet `client` → `in-house` / `agency` / `concept`. Legacy `output.tiles` removed.
