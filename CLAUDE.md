...
| The above-the-fold reveal contract | `src/components/PageHero.astro` — every page head, with `claims.mjs` failing the build on a hand-rolled one |
...
- **2026-07-29** — **The page hero is a component now** (`PageHero.astro`), because six of eleven templates had `.rev` above the fold and `.rev` is `opacity: 0`, which Chromium excludes from LCP: /brand measured 4238ms mobile LCP against 973ms FCP with its pixels on screen by 1144ms, and scored 100 on desktop throughout. The reveal sits on the wrapper, which is safe only because `.rev-load` is transform-only and never staggered. `claims.mjs` now fails the build on a hand-rolled hero, and **a new guard is not trusted until a known-bad case makes it fail** — the first draft of that check was inert, passing everything because `visibleCopy(src, file)` was called with the arguments reversed. Two more axes got scales: `--z-*` (nine hand-picked values ending 9998 and 99999) and the reading half of `--lh-*` (78 raw values across 23 numbers, frequency-derived so nothing moved more than 0.05). **`docs/` is in `TARGETS`** — it is tracked and the repo is public, so `docs/og-cards.md` had been naming the organization a rule already banned, against a guard pointed at nothing; docs get the claim and spelling rules but not the four `copyOnly` style rules, since pointing all 34 at them produced 111 violations that were almost entirely em dashes. Two rules in this file were **wrong rather than unenforced**: the `cqi` exemption was documented as "wired up nowhere" while wired correctly in three places, which is why nobody caught `PerfTable` measuring against the viewport; and "animate transform and opacity, nothing else" was contradicted by ten shipped rules including the theme switch, so it now bans layout properties instead. **`--surface-raised` is literal `#FFFFFF` on purpose** and says so where it sits: `--n-0` is the ramp's top and is already `--surface`, so a raised surface has nowhere warmer to go.
