...
- **JetBrains Mono** — OFL-1.1, no Reserved Font Name: both granted. `public/fonts/OFL.txt` ships beside it because OFL §2 requires the license to accompany the font. It ships whole anyway (see the decision log) — subsetting it is the first lever if PageSpeed ever demands one.
...
- **2026-07-27 (c)** — **Layout became a scale, and the sweep it exposed.** Asked for a best-practice pass on loading, found the failure mode the other scales exist to stop: the numbers describing the shell were retyped at every call site. Five distinct `sizes` strings shipped, four derived from one helper buried in `OutputGrid.astro` and **the two that were hand-written were the two that were wrong**, one of them on the LCP element of every case study. `src/lib/layout.ts` now owns the breakpoints, the shell inset (`rail + 2*pad`, so 392 / 64 / 48 can no longer disagree with the CSS), `sizesFor`, `FULL`, `sizesForSplit`, and the width ladders. Compose a helper, never type a literal, exactly like `--fs-*` and `--measure-*`. `/brand` §05 renders it live.
  - **The 12-column grid never existed.** `--grid-cols`, `--grid-max-width`, `--grid-gutter` and `--grid-margin` were read by nothing but `/brand`, which used them to describe a 1200px-capped 12-column grid with 24px gutters. Nothing on the site is capped, the real gap is 16px, and `.stage`/`.pad`/`.cs-wall` set no `max-width` at all, which is exactly why the hand-written `1100px` fallback was wrong above a ~1492px viewport. All four deleted; the page now documents the shell it actually has.
  - **Work-card clips were fetched on load, on every hover-capable device.** `wireHoverCards` gated `applyCardSources` on `canHover()` — capability, not gesture — so `<source>` + `load()` ran for every card at parse time: ~505KB on home, ~190KB on `/work`, whether or not anyone hovered. **Gate a fetch on the event, never on the capability.** `WorkIndex.astro` had had it right all along.
  - **No case study preloaded its LCP poster.** It sat ~10KB into the body behind the scoreboard, with three font preloads ahead of it, so the parser could not discover it until it had read past all of that. `Base.astro`'s `preloadImage*` trio existed and was proven on two routes; the seven `heroVideo` entries now pass it, from **one** `getImage()` shared with the `<img>` so the preload cannot resolve to a different candidate and fetch twice.
  - **35 poster stills moved out of `public/` into the image pipeline** (`src/assets/wc/`, `src/assets/ov/`), which §6 had always required and which also retired a live cache bug: they were unhashed under a year of `immutable`, so a re-export could never reach a returning visitor. **A hashed URL cannot be derived at runtime**, which is the whole shape of the change: the clip path is still built from the slug, but the server writes both poster variants onto the element as `data-poster` / `data-poster-light` and `card-video.ts` reads them. `<video poster>` takes one URL, so this bought hashing and format handling, **not** responsiveness.
  - **A conditional around a `<script>` does not remove it** — Astro hoists component scripts at build. Only a component that is never rendered drops its script, hence `OutputVideo.astro`: four entries have an Output section with no video block and were shipping a controller whose query always came back empty.
  - `favicon.ico` was **424,139 bytes** (nine images, seven uncompressed BMP, a 270KB 256x256) against 417 B for its own `favicon-32x32.png`. Regenerated to three PNG-compressed sizes, **1,434 bytes**; `scripts/favicon/render-ico.mjs` is the record. HTML, the resume PDF and `llms.txt` now declare `Cache-Control` instead of inheriting an unstated platform default. One shared `MutationObserver` serves every `onThemeChange` caller, where home and `/work` each registered two for one signal.
  - **`frc` is a 720p source, not a bad export** — its hero webm is 1280x720 too, and no master exists. Don't "fix" it. Six of the seven hero posters are 1920x1080 because their films are, so the full-width ladder's 2400 step only ever generates for `sportime-clubs`.
  - Cost, stated plainly: the case-study poster on a wide desktop goes ~87KB to ~155KB, because the old `sizes` under-declared the slot and was quietly serving a soft image. Mobile is unchanged. Perf is **not** re-measured; check CLS alongside LCP and confirm desktop holds 100 before restating any published figure.
