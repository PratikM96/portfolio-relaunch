...
- **2026-07-27 (d)** — **The "zero layout shift" claim retired, and the sweep confirmed on live.** A font-swap shift on `/brand` §01, where all three faces meet in one block, measured 0.009 desktop in two consecutive runs. Lighthouse's `layout-shifts` audit names the cause outright: `"Web font loaded"` for all three woff2 files, which is the 2026-07-23 trap recurring — the shift was always latent and only registered once the page was fast enough to swap inside the measured window. **The claim went, not the shift.** 0.009 is about a twelfth of Google's "good" threshold, the CLS audit still scores a perfect 1, and `/brand` still scores 100, so chasing it would have bought nothing a user or a tool can see. What it did cost was a page asserting "zero layout shift on every page" directly above its own table printing 0.009, on the one case study whose thesis is that the numbers are measured rather than claimed. Removed from ten surfaces (the entry's description, a decision, and its proof figure; the home bento blurb; two `/resume` strings; `llms.txt`; and the Resume Master's summary, systems bullet and Claim Registry row, which now carries a dated retirement note). **A perfect Lighthouse score already entails a good CLS**, so the figure was always a second number saying the same thing, and it was the one that could rot. `PerfTable` still renders per-page CLS as measured detail, which is where a number that moves belongs.
  - **The sweep in (c) is confirmed against the live site, not just the build.** Desktop home went 1914 to 1396 KiB with zero `/wc/*.webm` requests, and `/work` 399 to 205 KiB, both within a few KiB of the predicted 505 and 190. Home mobile recovered fully once the hero loop was held back: TBT 65 to 0, LCP 2991 to 2283, FCP 1508 to 1088. Desktop held 100 on all 23 pages for a third consecutive run.
  - **Single-run mobile LCP is not evidence.** The identity of the worst page rotated completely across three runs on three different builds: raa / agency-fiveeighty / the-ninth, then home / work / privacy, then jmtp / sportime / frc. Pages with no changes at all moved +/-750ms in both directions, and the two pages the sweep targeted improved *before* it shipped. The mean is stable (2065 / 2285 / 2153); per-page mobile is +/-700ms of noise. Read the mean, or read a CPU metric like TBT, which is what actually caught the hero's decode cost.
  - **The JS figure matched neither basis, and the proof box gained mobile.** `7KB` sat between home's 9,636 B raw and 4,529 B gzip, so it was wrong whichever way a reader took it. It is now **4.5KB transferred**, with the basis stated in the registry because raw and gzip differ by more than a rounding here. The mobile Lighthouse average took the slot CLS vacated, keeping the box at four figures. **Two of those four are hand-typed averages that `PerfTable` also renders from the JSON**, so both are rounded *down* (mobile 98.5 to 98, LCP 520ms to 0.5s) and both need re-checking against `portfolio-perf.json` after every run. If a gap ever reads as a contradiction rather than a rounding, move the figure into the generated table instead of retyping it — that is the failure the CLS figure just demonstrated.
