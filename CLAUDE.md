...
- **2026-07-27 (g)** — **A perf run is a sample set now, and the published row is the MEDIAN sample.** `run.sh` takes `REPEATS` loads per page per strategy (default 3, keep it odd) and `emit.mjs` picks the median rather than averaging. **Averaging each metric independently would publish a row that never happened** — an FCP from one load beside an LCP from another, and a score no real load produced — and metric distributions are right-skewed, so one slow sample drags a mean while barely moving a median. Same reason Lighthouse CI selects a median run. Ordered by score then LCP, and an even count takes the **lower** middle so the figure is never the flattering half of a coin flip. Rows carry `samples` and `lcpSpread`, each strategy carries a `sampling` summary, and `trend.mjs` prints `n` plus the within-run spread so a row's confidence is visible rather than assumed. **Sampling shrinks the ±600ms envelope, it does not remove it**: a median of 3 is roughly half the spread of one sample, so pages will still move a point. Verified against a staged 3-sample set with known values (91/97/94) to confirm it takes 94 and not the best or worst, and against the single-sample archive to confirm the legacy path still emits byte-identically. Cost is ~8 minutes and ~30MB per repeat.
  - **The published mobile average may move once a sampled run lands.** `98` in the proof box came from unsampled runs, and a median-of-3 average will differ slightly. Re-check it and the LCP figure against the JSON before restating either, per the rule in the registry.
