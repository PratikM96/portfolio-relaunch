...
- **2026-07-27** — **American spelling adopted and enforced.** Found by Pratik reading `/brand`, where `[ 04 ]` said "Color" and `[ 07 ]` said "Colour" — the page contradicted itself in two headings. The audit found 7 instances in rendered copy (four on `/brand`, `behaviour` in pipeline-medical's `constraints`, `programme` in raa's, `labelled` in `llms.txt`) and ~75 more across comments, docs, this file and the masters. All 86 normalized in one sweep, comments included, so no second dialect survives behind a `/* */`. The rule is §4 and is enforced by `scripts/check/claims.mjs`, which scans visible copy only. **Two traps if that rule is ever edited:** it must not match words correct in both dialects (advertising, raised, analysis, emphasis) or the loose stem `realis`, which hits "realistically"; and `public/fonts/OFL.txt` is third-party license text that OFL §2 requires to ship verbatim, so it is excluded, never corrected. A sitewide find-and-replace here is safe only because the British forms share no spelling with any CSS property (`color`, `scroll-behavior`, `color-mix` are already American) — check that before reaching for one again.
