...
Fonts are the repo's self-hosted woff2, referenced by paths **relative to the template**. Keep them relative — an absolute `file:///` path silently falls back to system fonts on another machine, and a wrong-font card looks fine until you compare it to the site.
...
Anything that changes every card — a font swap, a token change, a facet relabel — is one command. Needs Chrome (paths at the top of the script); renders at exactly 1200x630 into `public/og/`.
...
Wiring the render into the build (satori + resvg, or a headless step over the collections) would let posts self-generate. The script closes the reproducibility gap; this would close the "remember to run it" gap.
